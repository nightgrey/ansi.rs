...
use tree::{At, LayoutContext, LayoutNode, RootTree, Secondary, id, layout::prelude::*};
...
impl Document {
    ...
    pub fn insert_child_with_layout(
        &mut self,
        parent: ElementId,
        element: Element,
        layout: Layout,
    ...
    pub fn compute_layer(&mut self, id: ElementId, layer_id: LayerId) {
    ...
    pub fn compute_layouts(&mut self) {
        ...
        // Compute layout
        AsLayoutContext::compute_layout_with_measure(
            self,
            root_id,
            layout::Size {
            ...
            |known_dimensions, available_space, id, element, layout| {
                match &element.kind {
                    ElementKind::Span(content) => {
                        ...
                        let min_line_length: usize =
                            words.iter().map(|line| line.len()).max().unwrap_or(0);
                        ...
                        let block_size =
                            known_dimensions.get_abs(block_axis).unwrap_or_else(|| {
                                ...
                                for word in &words {
                                    ...
                                    } else if current_line_length + word.len() + 1
                                        > inline_line_length
                                    {
                    ...
                    _ => match (known_dimensions.width, known_dimensions.height) {
                        (Some(width), Some(height)) => layout::Size { width, height },
                        (Some(width), None) => layout::Size {
                            width,
                            height: f32::MAX,
                        },
                        (None, Some(height)) => layout::Size {
                            width: f32::MAX,
                            height,
                        },
                        (None, None) => layout::Size {
                            width: f32::MAX,
                            height: f32::MAX,
                        },
                    },
...
impl AsLayoutContext<ElementId, Element, Rect> for Document {
    ...
    ) -> LayoutContext<'_, ElementId, Element, MeasureFunction>
    where
