...
pub struct FlamegraphProfiler<'a> {
    options: Option<FlamegraphOptions<'a>>,
    frequency: c_int,
...
impl<'a> FlamegraphProfiler<'a> {
    ...
    pub fn with_options(mut self, options: Option<FlamegraphOptions<'a>>) -> Self {
        self.options = options;
        self
    }
}

impl Default for FlamegraphProfiler<'_> {
    fn default() -> Self {
        Self::new(100)
    }
}
        ...
        if let Some(profiler) = self.active_profiler.take() {
            if let Some(options) = self.options.as_mut() {
                profiler
                    ...
                    .build()
                    .unwrap()
                    .flamegraph_with_options(flamegraph_file, options)
