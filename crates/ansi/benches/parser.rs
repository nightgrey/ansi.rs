...
fn parser_throughput(c: &mut Criterion) {
    ...
    for (name, data) in &cases {
        for &scenario in SCENARIOS {
            bench(c, name, data, scenario);
        }
    }

    fn bench(c: &mut Criterion, name: &str, data: &[u8], chunk_size: Option<usize>) {
        let scenario = if let Some(chunk_size) = chunk_size {
            &format!("chunked/{}", ByteSize::b(chunk_size as u64).display().iec_short())
        } else {
            "whole"
        };
