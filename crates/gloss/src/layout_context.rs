...
use crate::{Display, FlexDirection, LayoutNode, Node, NodeId, Style};
...
where
    MeasureFunction: FnMut(taffy::Size<Option<f32>>, taffy::Size<taffy::AvailableSpace>, NodeId, & mut Node, &Style) -> taffy::Size<f32>,
{
...
where
    MeasureFunction: FnMut(taffy::Size<Option<f32>>, taffy::Size<taffy::AvailableSpace>, NodeId, & mut Node, &Style) -> taffy::Size<f32>,
{
    pub fn new(tree: &'d mut Tree<NodeId, Node<'n>>, layouts: &'d mut Secondary<NodeId, LayoutNode>, measure_function: MeasureFunction) -> Self {
        Self { tree, layouts, measure_function }
    }

    pub fn compute_layout(&mut self, id: NodeId, available_space: taffy::Size<taffy::AvailableSpace>)
    {
        let key = id.into_layout_id();
        taffy::compute_root_layout(self, key, available_space);
    ...
    MeasureFunction: FnMut(taffy::Size<Option<f32>>, taffy::Size<taffy::AvailableSpace>, NodeId, & mut Node, &Style) -> taffy::Size<f32>,
{
    ...
    fn child_count(&self, parent_node_id: taffy::NodeId) -> usize {
        self.tree.children(NodeId::from_layout_id(parent_node_id)).count()
    }

    fn get_child_id(&self, parent_node_id: taffy::NodeId, child_index: usize) -> taffy::NodeId {
        let child_key = self.tree.children(NodeId::from_layout_id(parent_node_id)).nth(child_index).unwrap();
        child_key.into_layout_id()
    }
}

impl<'d, 'n, MeasureFunction> taffy::TraverseTree for LayoutContext<'d, 'n, MeasureFunction>
where
    MeasureFunction: FnMut(taffy::Size<Option<f32>>, taffy::Size<taffy::AvailableSpace>, NodeId, & mut Node, &Style) -> taffy::Size<f32>,
{
}
    ...
    fn compute_child_layout(&mut self, node_id: taffy::NodeId, inputs: taffy::LayoutInput) -> taffy::LayoutOutput {
        ...
        taffy::compute_cached_layout(self, node_id, inputs, |ctx, node_id, inputs| {
            ...
            match (style.display, has_children) {
                ...
                (_, false) | (Display::Inline, _) => {
                    taffy::compute_leaf_layout(inputs, &style, |_, _| 0.0, |known_dimensions, available_space| {
                        (ctx.measure_function)(known_dimensions, available_space, node_key, node, &style)
                    })
                }
            }
        })
...
impl<'d, 'n, MeasureFunction> taffy::LayoutFlexboxContainer for LayoutContext<'d, 'n, MeasureFunction>
where
    MeasureFunction: FnMut(taffy::Size<Option<f32>>, taffy::Size<taffy::AvailableSpace>, NodeId, & mut Node, &Style) -> taffy::Size<f32>,
{
