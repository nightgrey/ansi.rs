...
macro_rules! position {
    ...
    ($value:expr) => {
      Position { row: $value as usize, col: $value as usize }
    };
}
...
macro_rules! point {
    ...
    ($value:expr) => {
      Point { x: $value as u16, y: $value as u16 }
    };
}
...
macro_rules! rect {
    ...
    (($min_x:expr, $min_y:expr), ($max_x:expr, $max_y:expr)) => {
        Rect::bounds(Point::new($min_x as u16, $min_y as u16), Point::new($max_x as u16, $max_y as u16))
    };
    ($min:expr, $max:expr) => {
        Rect::bounds($min, $max)
    };
