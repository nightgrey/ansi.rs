use crate::{Area, Bounded, Column, Contains, Point, Position, PositionLike, Rect, Row};

/// Trait to encapsulate behaviour to resolve a potentially context-dependent value
/// into a context-independent value.
pub trait Resolve<Into, Context> {
    /// Resolve a value.
    fn resolve(self, context: Context) -> Into;
}

// --- Area ---

// Position
impl Resolve<Position, Area> for Position {
    fn resolve(self, context: Area) -> Position {
       self
    }
}

impl Resolve<usize, Area> for Position {
    fn resolve(self, context: Area) -> usize {
        (self.row - context.min.row) * context.width() + (self.col - context.min.col)
    }
}

impl Resolve<Row, Area> for Position {
    fn resolve(self, context: Area) -> Row {
        Row(self.row - context.min.row)
    }
}

impl Resolve<Column, Area> for Position {
    fn resolve(self, context: Area) -> Column {
        Column(self.col - context.min.col)
    }
}

// PositionLike
impl Resolve<Position, Area> for PositionLike {
    fn resolve(self, context: Area) -> Position {
        self.into()
    }
}

impl Resolve<usize, Area> for PositionLike {
    fn resolve(self, context: Area) -> usize {
        (self.0 - context.min.row) * context.width() + (self.1 - context.min.col)
    }
}

impl Resolve<Row, Area> for PositionLike {
    fn resolve(self, context: Area) -> Row {
        Row(self.0 - context.min.row)
    }
}

impl Resolve<Column, Area> for PositionLike {
    fn resolve(self, context: Area) -> Column {
        Column(self.1 - context.min.col)
    }
}

// Row
impl Resolve<Position, Area> for Row {
    fn resolve(self, context: Area) -> Position {
        Position::new(self.value(), context.min.col)
    }
}

impl Resolve<usize, Area> for Row {
    fn resolve(self, context: Area) -> usize {
        (self.value() - context.min.row) * context.width()
    }
}

impl Resolve<Row, Area> for Row {
    fn resolve(self, context: Area) -> Row {
        Row(self.0 - context.min.row)
    }
}

impl Resolve<Column, Area> for Row {
    fn resolve(self, context: Area) -> Column {
        Column(0)
    }
}

// Column
impl Resolve<Position, Area> for Column {
    fn resolve(self, context: Area) -> Position {
        Position::new(0, self.value())
    }
}

impl Resolve<usize, Area> for Column {
    fn resolve(self, context: Area) -> usize {
        (self.value() - context.min.col)
    }
}

impl Resolve<Row, Area> for Column {
    fn resolve(self, context: Area) -> Row {
        Row(0)
    }
}

impl Resolve<Column, Area> for Column {
    fn resolve(self, context: Area) -> Column {
        self
    }
}

// Linear index
impl Resolve<Position, Area> for usize {
    fn resolve(self, context: Area) -> Position {
        Position::new(
            context.min.row + self / context.width(),
            context.min.col + self % context.width(),
        )
    }
}

impl Resolve<usize, Area> for usize {
    fn resolve(self, context: Area) -> usize {
        self
    }
}

impl Resolve<Row, Area> for usize {
    fn resolve(self, context: Area) -> Row {
        Row(self / context.width())
    }
}

impl Resolve<Column, Area> for usize {
    fn resolve(self, context: Area) -> Column {
        Column(self % context.width())
    }
}

// --- Rect ---

// Point
impl Resolve<Point, Rect> for Point {
    fn resolve(self, context: Rect) -> Point {
        self
    }
}

impl Resolve<usize, Rect> for Point {
    fn resolve(self, context: Rect) -> usize {
        (self.y - context.min.y) * context.width() + (self.x - context.min.x)
    }
}

impl Resolve<Row, Rect> for Point {
    fn resolve(self, context: Rect) -> Row {
        Row(self.y - context.min.y)
    }
}

impl Resolve<Column, Rect> for Point {
    fn resolve(self, context: Rect) -> Column {
       Column(self.x - context.min.x)
    }
}

// Row
impl Resolve<Point, Rect> for Row {
    fn resolve(self, context: Rect) -> Point {
        Point::new(context.min.x, self.value())
    }
}

impl Resolve<usize, Rect> for Row {
    fn resolve(self, context: Rect) -> usize {
        (self.value() - context.min.y) * context.width()
    }
}

impl Resolve<Row, Rect> for Row {
    fn resolve(self, context: Rect) -> Row {
        Row(self.0 - context.min.y)
    }
}

impl Resolve<Column, Rect> for Row {
    fn resolve(self, context: Rect) -> Column {
        Column(0)
    }
}

// Column
impl Resolve<Point, Rect> for Column {
    fn resolve(self, context: Rect) -> Point {
        Point::new(self.value(), 0)
    }
}

impl Resolve<usize, Rect> for Column {
    fn resolve(self, context: Rect) -> usize {
        self.value() - context.min.x
    }
}

impl Resolve<Row, Rect> for Column {
    fn resolve(self, context: Rect) -> Row {
        Row(0)
    }
}

impl Resolve<Column, Rect> for Column {
    fn resolve(self, context: Rect) -> Column {
        self
    }
}

// Linear
impl Resolve<Point, Rect> for usize {
    fn resolve(self, context: Rect) -> Point {
        Point::new(
            context.min.x + self % context.width(),
            context.min.y + self / context.width(),
        )
    }
}

impl Resolve<usize, Rect> for usize {
    fn resolve(self, context: Rect) -> usize {
        self
    }
}

impl Resolve<Row, Rect> for usize {
    fn resolve(self, context: Rect) -> Row {
        Row(self / context.width())
    }
}


// -- Referenced: Area --

// Position
impl Resolve<Position, &Area> for Position {
    fn resolve(self, context: &Area) -> Position {
        self
    }
}

impl Resolve<usize, &Area> for Position {
    fn resolve(self, context: &Area) -> usize {
        (self.row - context.min.row) * context.width() + (self.col - context.min.col)
    }
}

impl Resolve<Row, &Area> for Position {
    fn resolve(self, context: &Area) -> Row {
        Row(self.row - context.min.row)
    }
}

impl Resolve<Column, &Area> for Position {
    fn resolve(self, context: &Area) -> Column {
        Column(self.col - context.min.col)
    }
}

// PositionLike
impl Resolve<Position, &Area> for PositionLike {
    fn resolve(self, context: &Area) -> Position {
        self.into()
    }
}

impl Resolve<usize, &Area> for PositionLike {
    fn resolve(self, context: &Area) -> usize {
        (self.0 - context.min.row) * context.width() + (self.1 - context.min.col)
    }
}

impl Resolve<Row, &Area> for PositionLike {
    fn resolve(self, context: &Area) -> Row {
        Row(self.0 - context.min.row)
    }
}

impl Resolve<Column, &Area> for PositionLike {
    fn resolve(self, context: &Area) -> Column {
        Column(self.1 - context.min.col)
    }
}

// Row
impl Resolve<Position, &Area> for Row {
    fn resolve(self, context: &Area) -> Position {
        Position::new(self.value(), context.min.col)
    }
}

impl Resolve<usize, &Area> for Row {
    fn resolve(self, context: &Area) -> usize {
        (self.value() - context.min.row) * context.width()
    }
}

impl Resolve<Row, &Area> for Row {
    fn resolve(self, context: &Area) -> Row {
        Row(self.0 - context.min.row)
    }
}

impl Resolve<Column, &Area> for Row {
    fn resolve(self, context: &Area) -> Column {
        Column(0)
    }
}

// Column
impl Resolve<Position, &Area> for Column {
    fn resolve(self, context: &Area) -> Position {
        Position::new(0, self.value())
    }
}

impl Resolve<usize, &Area> for Column {
    fn resolve(self, context: &Area) -> usize {
        (self.value() - context.min.col)
    }
}

impl Resolve<Row, &Area> for Column {
    fn resolve(self, context: &Area) -> Row {
        Row(0)
    }
}

impl Resolve<Column, &Area> for Column {
    fn resolve(self, context: &Area) -> Column {
        self
    }
}

// Linear index
impl Resolve<Position, &Area> for usize {
    fn resolve(self, context: &Area) -> Position {
        Position::new(
            context.min.row + self / context.width(),
            context.min.col + self % context.width(),
        )
    }
}

impl Resolve<usize, &Area> for usize {
    fn resolve(self, context: &Area) -> usize {
        self
    }
}

impl Resolve<Row, &Area> for usize {
    fn resolve(self, context: &Area) -> Row {
        Row(self / context.width())
    }
}

impl Resolve<Column, &Area> for usize {
    fn resolve(self, context: &Area) -> Column {
        Column(self % context.width())
    }
}

// --- Referenced: Rect ---

// Point
impl Resolve<Point, &Rect> for Point {
    fn resolve(self, context: &Rect) -> Point {
        self
    }
}

impl Resolve<usize, &Rect> for Point {
    fn resolve(self, context: &Rect) -> usize {
        (self.y - context.min.y) * context.width() + (self.x - context.min.x)
    }
}

impl Resolve<Row, &Rect> for Point {
    fn resolve(self, context: &Rect) -> Row {
        Row(self.y - context.min.y)
    }
}

impl Resolve<Column, &Rect> for Point {
    fn resolve(self, context: &Rect) -> Column {
        Column(self.x - context.min.x)
    }
}

// Row
impl Resolve<Point, &Rect> for Row {
    fn resolve(self, context: &Rect) -> Point {
        Point::new(context.min.x, self.value())
    }
}

impl Resolve<usize, &Rect> for Row {
    fn resolve(self, context: &Rect) -> usize {
        (self.value() - context.min.y) * context.width()
    }
}

impl Resolve<Row, &Rect> for Row {
    fn resolve(self, context: &Rect) -> Row {
        Row(self.0 - context.min.y)
    }
}

impl Resolve<Column, &Rect> for Row {
    fn resolve(self, context: &Rect) -> Column {
        Column(0)
    }
}

// Column
impl Resolve<Point, &Rect> for Column {
    fn resolve(self, context: &Rect) -> Point {
        Point::new(self.value(), 0)
    }
}

impl Resolve<usize, &Rect> for Column {
    fn resolve(self, context: &Rect) -> usize {
        self.value() - context.min.x
    }
}

impl Resolve<Row, &Rect> for Column {
    fn resolve(self, context: &Rect) -> Row {
        Row(0)
    }
}

impl Resolve<Column, &Rect> for Column {
    fn resolve(self, context: &Rect) -> Column {
        self
    }
}

// Linear
impl Resolve<Point, &Rect> for usize {
    fn resolve(self, context: &Rect) -> Point {
        Point::new(
            context.min.x + self % context.width(),
            context.min.y + self / context.width(),
        )
    }
}

impl Resolve<usize, &Rect> for usize {
    fn resolve(self, context: &Rect) -> usize {
        self
    }
}

impl Resolve<Row, &Rect> for usize {
    fn resolve(self, context: &Rect) -> Row {
        Row(self / context.width())
    }
}


impl Resolve<Column, &Rect> for usize {
    fn resolve(self, context: &Rect) -> Column {
        Column(self % context.width())
    }
}

/// Trait to encapsulate behaviour to resolve a value from a context.
pub trait ContextualResolve<Into, With> {
    /// Resolve a value.
    ///
    /// Panics if the value is unable to resolve.
    #[inline(always)]
    fn resolve(self, with: With) -> Into;
}

impl<'a, Into, Context, T: Resolve<Into, &'a Context>> ContextualResolve<Into, T> for &'a Context {
    #[inline(always)]
    fn resolve(self, with: T) -> Into {
        with.resolve(self)
    }
}
