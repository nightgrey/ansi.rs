...
impl Bench {
    ...
    pub fn new(width: usize, height: usize) -> Self {
        Self {
            rasterer: Rasterer::new(width, height),
            arena: Arena::new(),
    ...
    pub fn with_capabilities(mut self, caps: Capabilities) -> Self {
        self.rasterer = self.rasterer.with_capabilities(caps);
        self
    }

    fn render(&mut self, buffer: &Buffer) {
        if self.buffer.width != buffer.width || self.buffer.height != buffer.height {
...
fn terminal_strict_ui(c: &mut Criterion) {
    ...
    for y in [10u16, 20, 30] {
        for x in 0..W {
            buf2[(y as usize, x)] = Cell::inline('*', upd);
        }
    }
...
fn terminal_table(c: &mut Criterion) {
    ...
    for &(row, col) in &[(5usize, 10usize), (12, 30), (20, 60), (30, 90), (38, 110)] {
